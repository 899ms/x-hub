...
**生成:** 2026-08-29 | **分支:** master | **版本:** 0.5.4
...
│   │   ├── lib.rs              # Tauri Builder：数据库/托盘/快捷键/窗口状态/单实例/数据迁移与恢复/128 命令注册/xhub-note 笔记图片协议/退出停 service
...
│   │   ├── chat_window.rs      # AI 对话独立窗口（label chat：按配置启动预创建隐藏常驻 + show/hide 快操作 + 几何防抖落盘 + 置顶持久化，见约定 33/41）
...
- **命令注册：** `src-tauri/src/lib.rs` 的 `invoke_handler!` 宏列出全部 128 个命令（前端封装一一对应 `src/api/tauri.ts`）
...
6. **浮窗存活实机自查（v0.5.3 事故后新增，见约定 41）**：本轮若动过 `notify.rs` / `clipboard.rs` / `floating_ball.rs` 或任何「审查修复」删过建窗代码，发版前实机跑一轮：设一条 1 分钟后的倒计时/待办提醒确认真右下角弹窗（通知窗是隐藏常驻窗，最容易被「删运行时 build」误杀成静默失联），并开关一次剪贴板浮层与悬浮球。
