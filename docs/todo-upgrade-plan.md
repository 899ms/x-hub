...
**第 2 项（已澄清）**：原话「待办要不要和速记一样也有一个卡片」指的是 **(b) 视图内布局**——速记视图是「卡片式」的（`NoteList.vue` / `NoteEditor.vue` 都是 `section.card`），希望待办视图也改成内容全部落在卡片里的样式。工作台模块卡（`todo_overview` / `todo` / `calendar`）不变。见 §12.2。

### 12.2 视图内卡片式布局（可单独撤销）

`TodoView.vue` 由「一整块自定义描边容器」改成与速记视图一致的两张卡：

- 左卡 `.card.tv-list`：视图头（标题 + 标签筛选 + 新建）+ 双维工具栏 + 分组列表（列表区单独滚动）
- 右卡 `.card.tv-cal`：日历头 + 图例 + 月/周网格
- 外框边距对齐 `.view-notes`（`padding: 0 20px 20px 0`）；`.card` 提供毛玻璃底/描边/圆角/阴影

**撤销方式**：本改动单独一个 commit，`git revert <该 commit>` 即可回到「整块容器 + 自定义描边」的旧版；与功能代码（拖拽/置顶/日历控件）不耦合。
