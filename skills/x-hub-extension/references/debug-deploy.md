...
脚手架仓库（`npm run new/validate/preview/deploy/pack`）是**内部开发环境才有**的东西，外部开发者手里没有。本 skill 自带 `templates/`，加上宿主的「开发者模式」，全流程都能走通：

| 环节 | 没有脚手架（默认情况） | 有脚手架（内部开发环境） |
|---|---|---|
| 生成骨架 | 复制本 skill 的 `templates/` 改字段 | `npm run new` |
| 校验 | 下面的自检清单 + 平台关卡清单 + **开发者模式真机跑一遍**（宿主扫描器的报错就是权威反馈） | `npm run validate` |
| 本地预览 | 浏览器直接打开入口 HTML（无桥，走 fallback 分支；主题也走兜底色） | `npm run preview`（mock 桥 + 主题注入） |
| 装进宿主 | **开发者模式**（更优：改代码 1.5 秒自动重载） | `npm run deploy` |
...
## 首选：宿主「开发者模式」（不需要任何工具，改完就能看）

1. 设置 →「**扩展 → 我的扩展**」→ 添加本机扩展源码目录（须含 `manifest.json`）；
2. 回「扩展」打开**开发者模式**；
3. 打开扩展即可跑真机——目录即真源（**不复制进已装列表**），**改代码约 1.5 秒自动重载**，可用真实数据与 service 后端；移除目录即撤销。
