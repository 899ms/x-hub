...
- **勾了 `module` → 必须再追问「模块标题」**：工作台卡片上显示的名字，写进 `manifest.name`。话术：`工作台上这张卡叫什么？默认「<你的建议名>」可以吗？`——标题是用户肉眼可见的文案，猜错就得返工，这一问不能省。
- **勾了 `module` 且要多形态**（同一张卡在不同格子里显示不同内容）：**也用多选**让他挑形态（如「通栏 / 宫格 / 紧凑」），再逐个问**形态名**（编辑器形态芯片上显示的字，写进 `moduleVariants[].name`）。把「模块标题 + 各形态名」**合并成一条清单**让他一次填完（同轮追问，不额外占提问轮次）。说不清就先给一个默认形态（min 2×2 / ideal 4×3），别硬造 4 个。
